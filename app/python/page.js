...
export default function Home() {
  ...
  const allModules = { ...curriculumPython };

  // Cargar progreso
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('py-learn-progress');
      if (saved) {
        try {
          ...
          // Cargar streak
          const streakData = localStorage.getItem('py-learn-streak');
          if (streakData) {
      ...
      // Actualizar streak
      localStorage.setItem('py-learn-streak', JSON.stringify({ lastDate: new Date().toISOString() }));
    }
  }, []);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('py-learn-progress', JSON.stringify({
        completedLessons
      }));
  ...
  const resetProgress = () => {
    if (confirm('¿Seguro que quieres reiniciar todo tu progreso?')) {
      setCompletedLessons([]);
      localStorage.removeItem('py-learn-progress');
    ...
    return (
      <div className="learn-container">
        ...
        <footer className="learn-footer">
          <p>🎓 Aprende Python paso a paso - {totalLessons} lecciones disponibles</p>
        </footer>
      </div>
    );
  }
